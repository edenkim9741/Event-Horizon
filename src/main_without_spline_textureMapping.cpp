...
void init() {
    ...
    // 텍스쳐 파일 로드 <- 절대경로 사용 주의!
    glEnable(GL_TEXTURE_2D);
    if (!sunTextureLoaded) {
        if (!loadSunTexture("/Users/omingyu/Cmake/Event-Horizon/texture/8k_sun.jpg")) {
            std::cerr << "Failed to load 2k_sun texture" << std::endl;
        }
    }
    if (!mercuryTextureLoaded) {
        if (!loadMercuryTexture("/Users/omingyu/Cmake/Event-Horizon/texture/8k_mercury.jpg")) {
            std::cerr << "Failed to load 2k_mercury texture" << std::endl;
        }
    }
    if (!venusTextureLoaded) {
        if (!loadVenusTexture("/Users/omingyu/Cmake/Event-Horizon/texture/8k_venus.jpg")) {
            std::cerr << "Failed to load 2k_venus texture" << std::endl;
        }
    }
    if (!jupiterTextureLoaded) {
        if (!loadJupiterTexture("/Users/omingyu/Cmake/Event-Horizon/texture/8k_jupiter.jpg")) {
            std::cerr << "Failed to load 2k_jupiter texture" << std::endl;
        }
    }
    if (!skyDomeTextureLoaded) {
        if (!loadSkyDomeTexture("/Users/omingyu/Cmake/Event-Horizon/texture/NightSkyHDRI009_8K_TONEMAPPED.jpg")) {
            std::cerr << "Failed to load sky dome texture" << std::endl;
        }
    }
    glDisable(GL_TEXTURE_2D);
}
